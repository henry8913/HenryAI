...
}  # dizionario con chiave = channel_id e valore = lista di messaggi
...
typing_habits = {
    "pauses": [0.3, 0.5, 0.7, 1.0,
               1.2],  # Pause casuali durante la digitazione
    "thinking_time": [0.5, 1.0, 1.5, 2.0,
                      2.5],  # Tempo di "riflessione" prima di rispondere
    "typing_speed": [0.01, 0.015,
                     0.02]  # Velocità di digitazione (secondi per carattere)
}

# Caratteristiche umane
human_traits = {
    "typos": [
        "ciao", "cia", "cioa", "hey", "ei", "ehi", "tutto bene", "tt bn",
        "come va", "tutto ok", "come procede"
    ],
    "fillers":
    ["uhm", "mmm", "beh", "ecco", "praticamente", "insomma", "diciamo"],
    "emoji_prob":
    0.4,  # Probabilità di usare emoji
    "reaction_prob":
    0.25,  # Probabilità di reagire ai messaggi con emoji
    "common_emoji": [
        "👍", "🙂", "😁", "😊", "👀", "🤔", "💯", "🔥", "👌", "🤩", "🤦‍♂️", "😅", "🚀",
        "💪", "🧐", "👏"
    ]
...
KEYWORD_RESPONSES = {
    ...
    "aiuto": [
        "Ti serve una mano? Dimmi pure", "Ci sono! Cosa ti serve?",
        "Eccomi, spara"
    ],
    "bloccato": [
    ...
    "ciao": [
        "Hey! Come butta?", "Ciao! Tutto a posto?", "Ehi, come va oggi?",
        "Ciao! Novità?"
    ],
    "buongiorno": [
...
async def analyze_code(code):
    """Analizza il codice fornito usando OpenRouter"""
    instruction = (
        "Analizza questo codice e fornisci un feedback conciso ma utile. "
        "Includi suggerimenti per miglioramenti e possibili problemi. "
        "Mantieni la risposta informale e amichevole."
    )
...
async def ask_openrouter(prompt, message_history=None, mentioned_users=None):
    ...
    data = {
        ...
        "messages": messages,
        "temperature":
        0.85,  # Temperatura più alta per risposte più creative e variabili
        "max_tokens":
        500  # Limitiamo la lunghezza per avere risposte più concise
    }
    ...
    try:
        ...
        if response.status_code == 200 and "choices" in response_data and response_data[
                "choices"]:
...
def update_conversation_history(channel_id, author_id, author_name, content):
    ...
    if len(conversation_history[channel_id]) > MAX_HISTORY_LENGTH:
        conversation_history[channel_id] = conversation_history[channel_id][
            -MAX_HISTORY_LENGTH:]
...
def maybe_add_filler(text):
    ...
    if random.random() < 0.3:  # 30% di probabilità
        ...
        return f"{filler}, {text.lower()}" if random.random(
        ) < 0.5 else f"{filler}... {text}"
    return text
...
def maybe_add_emoji(text):
    ...
    if random.random() < human_traits["emoji_prob"]:
        ...
        position = "end" if random.random(
        ) < 0.7 else "start"  # 70% alla fine, 30% all'inizio
...
def should_respond_to_message(message, message_content):
    ...
    if message.channel.id in last_response_time:
        ...
        if time_since_last < 30:  # Ridotto a 30 secondi per maggiore reattività
            if random.random(
            ) > 0.25:  # Aumentato a 25% di probabilità di rispondere in rapida successione
                return False

    # Lista di parole chiave che attivano una risposta
    trigger_keywords = list(KEYWORD_RESPONSES.keys())
    trigger_keywords.extend([
        "come", "perché", "cosa", "chi", "dove", "quando", "help", "codice",
        "errore", "bug", "javascript", "python", "html", "css", "framework",
        "sviluppo", "web", "frontend", "backend", "database"
    ])

    # Se è un messaggio che contiene una parola chiave
    if any(keyword in message_content for keyword in trigger_keywords):
        return random.random(
        ) < 0.75  # Aumentato a 75% di probabilità se c'è una parola chiave

    # Probabilità casuale di rispondere
    if random.random() < 0.35 and len(
            message_content.split()) > 2:  # Aumentato a 35%
        return True

    # Risponde se il messaggio è una domanda
    if "?" in message_content and len(message_content) > 10:
        return random.random(
        ) < 0.85  # Aumentato a 85% di probabilità per domande

    # Risponde se il messaggio ha un buon numero di parole (conversazione significativa)
    if len(message_content.split()) > 6:
        return random.random(
        ) < 0.45  # Aumentato a 45% di probabilità per messaggi lunghi
...
async def simulate_typing(channel, text):
    ...
    thinking_time = random.choice(
        typing_habits["thinking_time"]) * 0.7  # Ridotto del 30%
    if len(text) > 100:
        thinking_time *= 1.2  # Ridotto da 1.5 a 1.2

    # Attesa iniziale prima di iniziare a digitare
    await asyncio.sleep(min(thinking_time,
                            1.5))  # Cap a 1.5 secondi per essere più reattivo

    # Calcola il tempo di digitazione basato sulla lunghezza del testo
    typing_time = len(text) * random.choice(
        typing_habits["typing_speed"]) * 0.8  # Velocità aumentata del 20%

    # Aggiungi pause casuali per messaggi più lunghi
    if len(text) > 50:
        typing_time += random.choice(
            typing_habits["pauses"]) * 0.7  # Pause ridotte del 30%

    # Simula la digitazione
    async with channel.typing():
        await asyncio.sleep(
            min(typing_time,
                3.5))  # Cap ridotto a 3.5 secondi per essere più reattivo
...
async def on_message(message):
    ...
    if message.author != client.user and random.random(
    ) < 0.12:  # Aumentato dal 8% al 12%
        await maybe_add_reaction(message)

    # Aggiorna la cronologia della conversazione
    update_conversation_history(message.channel.id, message.author.id,
    ...
    if mentioned:
        ...
        if not prompt:
            choices = [
                "Dimmi tutto", "Che succede?", "Eccomi", "Mi hai chiamato?"
            ]
        ...
        response = await ask_openrouter(f"{instruction} Messaggio: {prompt}",
                                        channel_history, active_users)
        ...
        update_conversation_history(message.channel.id, client.user.id,
                                    client.user.name, response)

        # Aggiorna il timestamp dell'ultima risposta
        last_response_time[
            message.channel.id] = asyncio.get_event_loop().time()
        return
    ...
    for keyword in KEYWORD_RESPONSES:
        if keyword in lower_msg:
            if random.random(
            ) < 0.8:  # Aumentato dal 70% all'80% di probabilità
                ...
                if response:
                    ...
                    if random.random(
                    ) < 0.3:  # 30% di probabilità di taggare l'utente
                        response = f"<@{message.author.id}> {response}"

                    await message.channel.send(response)

                    # Aggiorna la cronologia e il timestamp
                    update_conversation_history(message.channel.id,
                                                client.user.id,
                                                client.user.name, response)
                    last_response_time[
                        message.channel.id] = asyncio.get_event_loop().time()
    ...
    # Gestione richieste di codice
    code_keywords = ["genera codice", "crea codice", "scrivi codice", "programma", "funzione per", 
                    "analizza codice", "controlla codice", "rivedi codice"]
    
    # Cerca blocchi di codice nel messaggio
    ...
    if should_respond_to_message(message, lower_msg):
        ...
        if random.random() < 0.25 and not any(f"<@{uid}" in response
